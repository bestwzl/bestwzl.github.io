...
function prev(elem){
	...
	//判断是否为元素节点（节点类型为1 代表是元素节点）
	if(elem.preciousSibling.nodeType == 1)  return elem.previousSibling;
	//继续往前找
...
/*--------6-----------addEventListener-------监听----------------*/
  /*addEventListener 添加事件
    @parem
      elem 元素
      type 事件类型
      fn   事件处理函数
      useCapture 是否捕获（默认）？ false是冒泡
*/
	...
	//判断元素是否支持  addEventListener
        ...
        //      //判断参数是否合法
        //      if(argument.length<3)return;
        //      //判断是否支持addEventListner方法
        //      if(elem.addEventListener){
